...
long maxDataPoints=50;
#ifdef ACCEL_DETAILS
long history[3][maxDataPoints];
#endif
...
while (!finished && !Serial.available()) {
    ...
    #ifdef ACCEL_DETAILS
    if((deltaL-prevDeltaL>15 && deltaR-prevDeltaR>15)){
        if(numberDataPoints<maxDataPoints){
            history[0][numberDataPoints]=positions[ENCODER_LEFT]; 
            history[1][numberDataPoints]=positions[ENCODER_RIGHT]; 
    ...
    #endif
    if(finished && millis()-lastChange>1000){
        break;
    }
    if (leftPower != 0 && abs(deltaL) >= abs(degrees)) {
        leftPower = 0;
    }
    if (rightPower != 0 && abs(deltaR) >= abs(degrees)) {
        rightPower = 0;
    }
    if ((abs(deltaL) >= abs(degrees) && abs(deltaR) >= abs(degrees))&&!finished) {
        finished = true;
        motorBrake(MOTOR_LEFT);
        motorBrake(MOTOR_RIGHT);
    }
    /*
    else if (deltaL - deltaR > 5) {
