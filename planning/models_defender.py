...
class GoToDefendPoint(Action):
    preconditions = [
        (lambda w, r: abs(utils.defender_rotation_to_defend_point(r.vector, w.ball.vector, w.our_goal.vector, GOAL_RADIUS, w.our_side)[0]) < ROTATION_DEFEND_POINT_THRESHOLD, "Defender is facing defending point"),
        (lambda w, r: abs(utils.defender_distance_to_defend_point(r.vector, w.ball.vector, w.our_goal.vector, GOAL_RADIUS, w.our_side)) > DEFEND_POINT_DISTANCE_THRESHOLD, "Defender far away from defending point"),
        (lambda w, r: r.catcher == 'OPEN', "Grabbers are open.")
    ]
